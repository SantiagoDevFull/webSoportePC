...
function CargarCarrito() {
    ...
    $('#carrito').load("ControlBoleta?accion=AgregarProducto&txtIDpro=" + idPro + "&txtNomPro=" + nombre + "&txtPrecioPro=" + precio + "&txtCantidadPro=" + cantidad,
            function () {
                $('#tabla3').DataTable(); // Actualizar tabla de detalles de la boleta
                $('#producto').load("ControlBoleta?accion=ActualizarProducto"); // Recargar lista de productos
            });
...
function Limpiar() {
    ...
    $('#carrito').load('ControlBoleta?accion=RehacerBoleta', function () {
        $('#tabla3').DataTable();
    });

    $('#producto').load('ControlBoleta?accion=RehacerBoleta', function () {
        $('#tabla2').DataTable();
        $('#tabla3').DataTable(); // Actualizar tabla de detalles de la boleta
        $('#producto').load("ControlBoleta?accion=ActualizarProducto"); // Recargar lista de productos
    });
...
function ValidarProceso() {
    ...
    $.ajax({
        type: 'GET',
        url: 'ControlBoleta?accion=DetalleSize',
        async: false,
...
function CargarDetalle(id){
    ...
    $('#detalle').load('ControlBoleta?accion=ListarDetalle&id='+id);
